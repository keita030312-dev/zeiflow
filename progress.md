...
- [x] 追加消化（同日）: 2FA有ログイン（3パターン）・10MB超エラー・OCR実走E2E（本番で全項目正読→仕訳作成）・ポータル送信→仕訳一覧→確定・PR #14 マージ済み
- [ ] 残: OCR**精度実測**のみ（実レシート10〜20枚が必要）。運用系: Resend独自ドメイン検証（🔴リセットメール不達、bugs.md参照）
