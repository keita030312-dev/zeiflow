...
- [x] **恒久対応=Vercel Blob移行も実装完了(PR #20)**: 画像本体をBlobへ・DBは参照URLのみ。Blob失敗時DBフォールバック・移行前レシート後方互換・削除全6経路でBlob掃除・URL露出防止。ストア`zeiflow-receipts`作成・トークン全環境設定済み。レビュー3巡「マージ可」。スタックPR(#19ベース)
- [ ] 残: PR #19→#20マージ+本番反映 → **デプロイ後に顧客非利用時間帯で `node scripts/migrate-receipt-images-to-blob.mjs` 実行**(既存97枚をBlobへ・最後のVACUUM FULL中は数十秒停止) → 顧客へ再送案内
