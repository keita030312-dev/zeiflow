...
## 📋 2026-07-28 顧客問い合わせ対応: CSV取込の「無効な日付」誤解を解消(ブランチ `fix/import-skip-aggregate-rows`)

- [x] **原因特定**: 弥生の帳簿エクスポート(仕訳日記帳)に混ざる集計行([日計行][月計行]=日付が空)・複合仕訳の相手側行(金額が空)が1行ずつ「無効な日付/無効な金額」の赤エラー表示になり、明細行は取り込めているのに顧客が「取込失敗」と誤解して問い合わせ(リスクマネジメント社 2026-07-28)
- [x] **修正**: `parseImportRows` を `journal-csv.ts` に抽出し、日付or金額が空欄の行はエラーでなく「対象外(skipped)」に分類。UIは「取込N件(緑)/対象外M件(青info)/エラーK件(赤)」の3区分表示。集計行のみのCSVは「合計行のみのCSVのようです」と説明を返す。実顧客ファイル構造(Shift-JIS・前置き行・31列・和暦)を再現した単体テスト6件追加(計44件全パス)・本番ビルド通過・独立レビュー「マージ可」

## 📋 2026-07-24 顧客要望対応: ドラッグ&ドロップ取込(PR #23・本番反映済み)

- [x] **PCからのドラッグ&ドロップで画像を取り込めるようにした**(顧客要望)。ダッシュボード=「撮影・アップロード」カード全体、ポータル=アップロード枠がドロップ対象。ドラッグ中は金色ハイライト+文言切替。検証(形式/10MB/5枚)は既存クリック選択と共通の`addFiles`に一本化。誤ドロップのページ遷移ガード・処理中/カメラ中の拒否・テキストドラッグ無視も実装。実ブラウザでDataTransfer実発火検証全PASS・レビュー2巡「マージ可」
...
- [x] PR #17 マージ+本番デプロイ完了(2026-07-23)。本番バンドルに新コードが配信されていることを実測確認済み。**LGTM(bot)+全CIグリーンで顧客環境に反映済み**
